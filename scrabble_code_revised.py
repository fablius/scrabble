    ...
    def check_word(self):
        ...
        if self.word != "":
            ...
            if self.word not in dictionary:
                print("\n" + "Please enter a valid dictionary word!.")
                print("Your score has been deducted.")
                self.player.decrease_score()
                print(self.player.get_name() + "'s score is: " + str(self.player.get_score()))
                return "try again!\n"
...
def turn(player, board, bag):
    ...
    # Determine the minimum score threshold for this round
    if round_number == 0:
        score_threshold = round_number * 0
    else:
        score_threshold = (round_number-1) * 5

    # Check if the player's score meets the threshold
    if player.get_score() < score_threshold:
        print(f"{RED_TEXT}{player.get_name()} has been eliminated for not reaching {score_threshold} points in round {round_number}.{RESET_TEXT}")
        other_player = [p for p in players if p != player][0]
        print(f"{CYAN_TEXT}{other_player.get_name()} wins the game!{RESET_TEXT}")
        players.remove(player)  # Remove the player from the game

        # If all players are eliminated, end the game
        if len(players) == 0:
            print(f"{RED_TEXT}All players have been eliminated! The game is over.{RESET_TEXT}")
            end_game()
            return
    ...
    if (skipped_turns < 6) or (player.rack.get_rack_length() == 0 and bag.get_remaining_tiles() == 0):
        ...
        while True:
            ...
            print("Press 5 to show letter values")
            ...
            if numm == "1":
                ...
                print("Scrabble Letter Values:\n")
                for letter, value in LETTER_VALUES.items():
                    print(f"{letter}: {value}", end="\t")
                    if (ord(letter) - ord('A') + 1) % 6 == 0:  # Break line after every 6 letters 
                        print()
                print("\n")
...
def start_game():
    ...
    # Setup players
    print(f"\n{CYAN_TEXT}Welcome to Scrabble!{RESET_TEXT}")
    print(f"{YELLOW_TEXT}Scoring Requirements:{RESET_TEXT}")
    print(f"{LIGHT_BLUE_TEXT}Round 2:{RESET_TEXT} Minimum 5 points")
    print(f"{LIGHT_BLUE_TEXT}Round 3:{RESET_TEXT} Minimum 10 points")
    print(f"{LIGHT_BLUE_TEXT}Round 4:{RESET_TEXT} Minimum 15 points")
    print("And so on, increasing by 5 points for each subsequent round.")
    print(f"{RED_TEXT}Entering an incorrect word will result in a deduction of 2 points.{RESET_TEXT}\n")
    print(f"{CYAN_TEXT}Please enter the names of the players below.{RESET_TEXT}")
    players = []
